...
namespace structures
{
	...
	{
		if ((*children_)[order] == nullptr && order < K)
		{
	...
	inline TreeNode<T>* KWayTreeNode<T, K>::replaceSon(TreeNode<T>* son, int order)
	{
		if (order < K)
		{
			TreeNode<T>* node = (*children_)[order];
			node->resetParent();
			(*children_)[order] = dynamic_cast<KWayTreeNode<T, K>*>(son);
			son->setParent(this);
			return node;
		...
		{
			throw std::range_error("Invalid order");
		}
	}
	...
	inline TreeNode<T>* KWayTreeNode<T, K>::removeSon(int order)
	{
		...
		{
			TreeNode<T>* node = (*children_)[order];
			if (node != nullptr) { node->resetParent(); }
			return node;
		}
		else
		{
