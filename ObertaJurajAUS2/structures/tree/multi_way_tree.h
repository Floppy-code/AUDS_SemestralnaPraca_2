...
namespace structures
{
	...
	{
		TreeNode<T>* temp = (*children_)[order];
		(*children_)[order] = dynamic_cast<MultiWayTreeNode<T>*>(son);
		son->setParent(this);
		if (temp != nullptr) { temp->setParent(nullptr); };
		return temp;
	}
	...
	{
		if (order < children_->size())
		{
			TreeNode<T>* temp = children_->removeAt(order);
			temp->resetParent();
			return temp;
		}
		else
		{
			throw std::range_error("Bad order");
		}
	}
