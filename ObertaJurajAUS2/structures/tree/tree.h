...
namespace structures
{
	...
	inline TreeNode<T>* TreeNode<T>::deepCopy()
	{
		...
		{
			if (result->getSon(i) != nullptr)
			{
				result->replaceSon(result->getSon(i)->deepCopy(), i);
			}
		}
		return result;
	}
	...
	{
		size_t sum = 1;		//PreOrder
		for (size_t i = 0; i < degree(); i++)
		{
			TreeNode<T>* son = getSon(i);
			if (son != nullptr) { sum += son->sizeOfSubtree(); }
		}
		return sum;
	}
	...
	inline Iterator<T>& Tree<T>::TreeIterator::operator=(const Iterator<T>& other)
	{
		...
		*TreeIterator::path_ = *otherTI.path_;
		return *this;
	}
	...
	{
		const TreeIterator& otherTI = dynamic_cast<const TreeIterator&>(other);
		return *TreeIterator::path_ != *otherTI.path_;
	}
