...
namespace structures
{
	...
	{
		return new InOrderTreeIterator(KWayTree<T, 2>::root_);
	}
	...
	{
		BinaryTreeNode<T>* currNode = dynamic_cast<BinaryTreeNode<T>*>(current);
		if (currNode != nullptr) {
			populatePath(currNode->getLeftSon());
			Tree<T>::TreeIterator::path_->push(currNode);
			populatePath(currNode->getRightSon());
		}
	}
