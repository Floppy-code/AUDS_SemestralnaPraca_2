...
namespace structures
{
	...
	{
		for (int i = 0; i < size_; ++i) if ((*array_)[i] == data) return i;
		return -1;
	}
	...
	inline void ArrayList<T>::reserve(size_t capacity)
	{
		if (capacity != capacity()) {
			if (capacity < size_) throw std::exception("Invalid capacity!");
			else {
				Array<T>* temp = new Array<T>(capacity);
	...
	{
		if (capacity() != size_) {
			Array<T>* temp = new Array<T>(size_);
			Array<T>::copy(*array_, 0, *temp, 0, size_);
			delete array_;
			array_ = temp;
		}
