...
namespace structures
{
	...
	{
		bool found = false;
		BSTTreeNode* BSTNode = findBSTNode(key, found);
		if (found)
		{
			T data = BSTNode->accessData()->accessData();
			extractNode(BSTNode);
			delete BSTNode;
			--size_;
			return data;
		...
		{
			throw std::logic_error("No such node in tree!");
		}
	}
	...
	inline typename BinarySearchTree<K, T>::BSTTreeNode* BinarySearchTree<K, T>::findBSTNode(const K & key, bool & found) const
	{
		...
		else
		{
			...
			{
				if (lastNode->accessData()->getKey() > key)
				{
	...
	{
		BSTTreeNode* substitute = nullptr;
		BSTTreeNode* oldParent = node->getParent();

		if (node->hasLeftSon() == true && node->hasRightSon() == false)	//Jeden syn
		{
			substitute = node->getLeftSon();
			node->setLeftSon(nullptr);
		}
		else if (node->hasLeftSon() == false && node->hasRightSon() == true)
		{
			substitute = node->getRightSon();
			node->setRightSon(nullptr);
		}

		if (node->hasLeftSon() == true && node->hasRightSon() == true)	//Dvaja synovia
		{
			substitute = node->getRightSon();	//Nastavenie na pravy podstrom
			while (substitute->hasLeftSon())	//Prechadza pravy prodstrom kym nenajde najmensi prvok
			{
				substitute = substitute->getLeftSon();
			}
			extractNode(substitute);	//Vyberie node(list)

			BSTTreeNode* leftSubtree = node->getLeftSon();	//Vyberie pravy podstrom
			node->setLeftSon(nullptr);						//Zrusi odkaz nan v node
			substitute->setLeftSon(leftSubtree);			//Prida vybraty node ako root podstromu

			BSTTreeNode* rightSubtree = node->getRightSon();
			node->setRightSon(nullptr);
			substitute->setRightSon(rightSubtree);
		}

		if (oldParent == nullptr)	//Ak je node koren
		{
			binaryTree_->replaceRoot(substitute);
		}
		else
		{
			if (oldParent->getLeftSon() == node)
			{
				oldParent->setLeftSon(substitute);
			}
			else
			{
				oldParent->setRightSon(substitute);
			}
		}
		if (substitute != nullptr)	//Prida nahrade parenta povodneho node
		{
			substitute->setParent(oldParent);
		}
	}
}