...
namespace structures
{
	...
	inline void Treap<K, T>::rotateLeftOverParent(BinarySearchTree<K, T>::BSTTreeNode * node)
	{
		...
		{
			BSTTreeNode* leftNode = node->getLeftSon();
			BSTTreeNode* rootNode = node->getParent();
			BSTTreeNode* rootRootNode = rootNode->getParent();
			node->setLeftSon(nullptr);
			rootNode->setRightSon(nullptr);
			if (rootRootNode != nullptr)
			{
				if (rootNode->getParent()->getLeftSon() == rootNode)
				{
					rootRootNode->setLeftSon(node);
				}
				else
				{
					rootRootNode->setRightSon(node);
				}
			}
			rootNode->setRightSon(leftNode);
			node->setLeftSon(rootNode);
		}
		else
		{
			throw std::logic_error("Node is not a left son!");
		}
	}
	...
	inline void Treap<K, T>::rotateRightOverParent(BinarySearchTree<K, T>::BSTTreeNode * node)
	{
		...
		{
			BSTTreeNode* rightNode = node->getRightSon();
			BSTTreeNode* rootNode = node->getParent();
			BSTTreeNode* rootRootNode = rootNode->getParent();
			node->setRightSon(nullptr);
			rootNode->setLeftSon(nullptr);
			if (rootRootNode != nullptr)
			{
				if (rootNode->getParent()->getLeftSon() == rootNode)
				{
			...
			rootNode.setLeftSon(rightNode);
			node->setRightSon(rootNode);
		}
		else
		{
			throw std::logic_error("Node is not a right son!");
		}
	}
}