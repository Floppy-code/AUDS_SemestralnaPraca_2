...
namespace structures
{
	...
	{
		if (indexStart == size())	//Kontrola pravej strany
		{
			found = false;
			return indexStart;
		}

		size_t indexMiddle = (indexStart + indexEnd) / 2;
		K keyMiddle = (*SequenceTable<K, T>::list_)[indexMiddle]->getKey();
		...
		else
		{
			...
			{
				found = false;
				return keyMiddle < key ? indexMiddle + 1 : indexMiddle;
			}
			else
			{
				if (key < keyMiddle)
