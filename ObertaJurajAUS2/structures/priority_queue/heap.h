...
namespace structures
{
	...
	T Heap<T>::pop()
	{
		...
		if (size() > 1) {
			...
			while ((sonIndex < size()) && ((*list_)[sonIndex]->getPriority() < item->getPriority())) {
				...
				index = sonIndex;
				sonIndex = getGreaterSonIndex(index);
			}
		}
		else {
			list_->clear();
	...
	{
		int indexL = 2 * index + 1; //((index + 1) * 2) - 1;
		int indexR = 2 * index + 2; // ((index + 1) * 2);
	...
	{
		if (size() == 0) throw std::logic_error("Priority queue is empty!");
		return 0;
	}
}